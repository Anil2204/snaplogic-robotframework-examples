...
Verify Pipeline Execution Completed Successfully
    ...
    ...    Record count mismatch! Expected ${source_count}, got ${target_count}


Validate Data Integrity - Sample Records Match
    [Documentation]    Validates that sample records from source and target are identical after mapping.
    ...
    ${target_record}=    Execute SQL Query And Get Results
    ...    SELECT metric_name, metric_unit, month, platform FROM ${target_table} LIMIT 1    ${FALSE}
    
    Should Not Be Empty    ${target_record}    No target records after load
    
    ${tgt_row}=    Get From List    ${target_record}    0
    Log    Target sample record: ${tgt_row}
...
Validate No Duplicate Records in Target
    ...
    ...    SELECT COUNT(DISTINCT metric_name, metric_unit, month, platform) FROM ${target_table}
